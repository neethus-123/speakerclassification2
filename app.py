...
with header:
    ...
    datapath1=  "C:\\Users\\Neetha Ravindran\\Downloads\\audio\\neetha\\"
    datapath2= "C:\\Users\\Neetha Ravindran\\Downloads\\audio\\neetha\\"
#def identify():
...
for i in range(1,5):
    path ="C:\\Users\\Neetha Ravindran\\Downloads\\audio"
    header += f' mfcc{i}'
header += ' label'
header = header.split()
print(header)
...
#model.add(layers.Dense(13,activation='lstm'))
...
if(CREATE_CSV_FILES==True):
    extractwavFeatures("C:\\Users\\Neetha Ravindran\\Downloads\\audio\\neetha",TEST_CSV_FILE)
import pandas as pd
import csv
...
def printPrediction(x_data,y_data,printDigit):
    ...
    for i in range(len(y_data)):
        ...
        prediction=getSpeaker(model.predict(x_data[i:i+1])[0].argmax(axis=-1))
        #prediction=model.predict(np.expand_dims(x_data[i:i+1],axis=0))
